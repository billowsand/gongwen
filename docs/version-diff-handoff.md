...
| ② | 左 Markdown diff / 右花脸稿预览的界面、标记绘制、联动、打印预览 | **大部分完成**，已合入 `main`；剩「公文要素就地标注」（**下一步**，见下文） |
| ③ | 可编辑 diff + 逐块还原 | 未开始 |
...
1. **补完第 ② 期：公文要素就地标注**（做法见上文「第 ② 期还没做的」）。
2. 人眼核对预览里的标记位置（见「已知的坑」），必要时微调 `marks.rs` 常量。
3. 第 ③ 期：可编辑 diff + 逐块还原。最大风险见方案第五节（在 egui `TextEdit` 上画
   不属于文本的「已删除行」）。届时：
   - 左栏从 `unified_body_ui` 换成编辑器本身，`ChangeLinks` 照用；
   - 视觉 diff 挪到后台线程 + 150ms 防抖（见「与方案的出入」第 4 条）；
   - 「还原」要走编辑器撤销栈。
