...
| ③ | 可编辑 diff + 逐块还原 | **已完成**（见「第 ③ 期交付了什么」）；人眼验收待做 |
...
- （第 ③ 期）**视觉 diff 引擎的已知问题（第 ① 期遗留，未修）**：一处「删掉一段」与远处
  「新增一段」会被文字流比较拼到同一处做整句替换。例：旧版甲乙丙丁戊五段，新版删丁、在甲后
  新增一段，花脸稿把丁画成删除线放在甲后面、与新段拼成一句，而不是在丙戊之间。小稿同样
  复现，与本期的锚定提速无关（两条路径结果相同）。修的时候注意两侧还原不变式。
...
1. **第 ③ 期人眼验收**：长稿边打字边看（右栏不卡、左栏不跳），空隙里的旧行、绿底、
   还原按钮的观感，打印预览的 PDF 与右栏一致。
2. 视觉 diff 引擎「远处删一段 + 增一段被拼成一处」的问题（见「已知的坑」）。
3. 公文要素就地标注（第 ② 期遗留，做法见上文），要在第 ④ 期之前补上。
