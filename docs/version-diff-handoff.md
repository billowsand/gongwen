...
- 右栏单击带标注的块 → 左栏聚焦并滚到那处变更；点未改动的块 → 预览里标亮，左栏滚到同一行并描边
  （藏在折叠里就先展开，`DiffViewState::locate_context`）。
...
- 右栏点带标注的块 → 焦点 + 光标跳到那一块。
- 未改动的文字也双向联动（第 ④ 期后补）：光标换到没改动的行 → 右栏标亮并滚到同一段
  （`ChangeLinks::marked_for_new_source`）；右栏点没改动的段 → 光标跳到那一段段首
  （`ChangeLinks::new_source_for_marked`）。花脸稿还没按当前正文重算好时不联动，免得偏移错位；
  光标是被预览点击带过来的，右栏不再二次滚动。
...
用户反馈：页数一多（60 页以上）预览就卡，版本对照模式更明显。实测（release，约 4 万字 ≈ 60 页）：
预览每帧整篇重排，右栏约 6 ms/帧、随篇幅线性增长；对照模式右栏没有接「宽度在变时冻结版面」，
拖分隔条 / 收起时间轴 / 缩放窗口时每帧按新字号整篇重排并重新栅格化，约 115 ms/帧，
60 帧里字体图集被撑满重建 4 次。改了四处：

- `preview::freeze`（`ScaleFreeze` + `show_frozen`）：从起草页 `markdown_render` 抽出的
  「宽度还在变就沿用落定的版面、只做层变换」，起草页公文预览、版本对照右栏、稿件管理
  对照窗共用。拖动时 115 ms → 1.2 ms/帧。
- `preview::memo`：按内容哈希缓存每帧重复的纯计算，挂在 egui 临时数据上、每类留 4 份。
  公文预览的解析 + 切块、研究报告的两遍解析都走它（60 页约省 1.2 ms/帧）。
- `preview::cull`：公文 / 研究报告的正文块画完记下高度与号栏行，远离视野（可视区上下各
  放宽一屏之外）的块只按记录占位、不排版。键是「块内容（不含源码绝对位置）+ 标题编号
  计数器（仅标题）+ 缩放 / 版心 / 字面 / 号栏开关」。跳过的标题照样推进计数器、登记导航
  要回查的矩形；号栏行按记录补回。右栏 6.2 ms → 0.3 ms/帧。
- 红头呈批件：分页结果整份缓存（键含表单 JSON、各显示值、要素标注、缩放、配色版本与
  `memo::font_epoch`），可视区附近之外的纸只占位；开着页边行号或锚点待滚动的那页照画。
  6.4 ms → 0.12 ms/帧。

结果：60 页的对照模式整页 7.3 ms → 1.3 ms/帧，120 页 17.3 ms → 2.7 ms/帧；剩下的大头是左栏
diff 编辑器（每帧给全部源码行画行号、铺底色，60 页约 0.6 ms），还没做可视区裁剪。
测试见 `preview::cull::tests`、`preview::memo::tests`、
`version_diff::tests::preview_scale_stays_frozen_while_the_width_is_changing`。
...
- （预览性能）**跨帧留着的 galley 在字体图集重建后就是乱码**：图集里的字形坐标整套换了。
  缓存 galley 的地方要么每帧向 epaint 重新要（`highlight::cached_galley` 的做法），要么把
  `preview::memo::font_epoch` 放进缓存键（红头呈批件分页缓存的做法）。`preview::cull` 只存
  高度与号栏行、不存 galley，没有这个问题。
- （预览性能）`preview::cull` 的键不含字体设置：换字体后远处块的占位高度可能暂时不准，
  块一滚进视野就现排、记录随即更新，只表现为滚动条长度微调。锚点所在块、看得见的块永远现排。
- （预览性能）红头呈批件开着页边行号时不跳过任何一页：号码整篇一路数下来，跳页就要按记录
  补回每一行的号，打印分页器目前没做这份记录。
...
6. ~~长稿右栏预览按可视页裁剪~~（已做，见「预览长稿性能」）。左栏 diff 编辑器的行号与底色
   仍是整篇画，120 页约 1.6 ms/帧，需要时再按可视行裁剪。
