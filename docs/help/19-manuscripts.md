...
外部的 txt / csv / md 会自动识别编码：有 BOM 按 BOM，整份是合法 UTF-8 按 UTF-8，否则交给编码探测（GBK 与 Big5 拿不准时偏向 GBK）——GBK、UTF-16 的老文件不必先另存为 UTF-8。程序自己写出的配置、TeX 与稿件库仍按 UTF-8 读。
