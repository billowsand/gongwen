...
| 分栏 | 左源码、右版式并排 | 对照着改 |
| 版本对照 | 左栏 diff（可改字、可逐块还原），右栏花脸稿预览 | 看这次改了什么，顺手就地还原 |
...
编号与合并由程序生成，**源码里看不到**：预览（实时排版、分栏）与导出的 Word / PDF 里才有。这与 `##` 标题的自动编号是同一套做法。各文种（含红头呈批件、研究报告）都支持。

### 居中与居右

正文默认首行缩进、两端对齐。个别行要居中或靠右（题词、引文出处、特殊落款之类），在它们上面加一行标记：
...
- 标记下方**直到空行**为止的每一行都按标记排：居中是整行在版心居中，居右是整行贴右边，都**不缩进**；
- 区里**每行单独成一行**，不会像正文那样把相邻几行接成一段；
- 区里只认行内格式（加粗、括号楷体），`#`、`1.`、表格竖线都当普通文字；
- 区里再写一个标记，从那一行起就换成新的对齐方式；标记行本身不印到纸上；
- 也可以写成 `<!-- [右对齐] -->`，或英文的 `center` / `right`；
- 不用手敲：选中这几行，点「格式」分区的「居中」或「居右」按钮即可，再点一次取消。

预览、Word、PDF 三处排法一致。各文种（含红头呈批件、研究报告）都支持。
