...
- 起草页按「开始、插入、格式、审校、视图、输出」六个分区组织功能区，插入表格（网格选行列、行列增删、横纵单元格合并/拆分、列对齐）、标题层级、加粗、词库词条、中文日期等都在其中；保存、提交版本、导出常驻标题栏；
...
- Markdown 源码编辑器可按元素分别选字面：预置「编辑器字体」「标题随公文」「与公文一致」三套模板，也可给文档标题、各级标题、正文与标记逐项指定小标宋、黑体、楷体、仿宋，源码看起来就与成稿一致；
- 内置稿件库，支持草稿、发布、归档，以及版本历史、附件和 ZIP 导出/导入；
- 内置「公文词表」：从已发布与已归档的稿件、知识库文档里长期累积词频，合并标准词库的单位人员与校对词表的建议写法，导出小鹤双拼用户码表；同一份词表还会挂成 jieba 用户词典，让检索召回与标题断行认识本单位专名；
