...
- **PDF 应用内预览**：纯 Rust 渲染，连续滚动与单页翻页两种模式——单页模式下窗口够宽就并排多页、翻页按整屏步进，触控板滑一下翻一格；跳页、缩放齐备，导出的成品和归档的盖章扫描件都能在标签页里直接翻看。
- **导入旧材料**：Word / Excel / PPT / ODF / RTF / EPUB / CSV 等 23 种格式可直接转成 Markdown 新建文档；外部 txt / csv / md 自动识别编码，GBK、UTF-16 的老文件不必先另存为 UTF-8。
- **界面随人**：界面字体直接用本机字体，不随应用打包；主题见下文「界面一览」。

## 文稿管理：从起草到归档的全流程

- **版本管理**：随时提交版本（带名称和备注），任意两版增删对照；对照视图左栏是可编辑的统一 diff（删除的旧行原位红底、新行绿底，每个变更块一键还原），右栏同步出花脸稿预览；「导出花脸稿」把删掉的字留在原位画红色删除线、新增的字就地套蓝色方框，字体行距与定稿完全一致；可一键回退到基准版。
...
- **AI 技能包**：把本程序的 Markdown 公文子集、七种文种的正文规则、行文规范与文种模板打成一套 Agent Skill（`gongwen-markdown`），设置页「上手指引」可导出 `.skill` 或技能文件夹，装进 Claude Code、Codex 等外部工具后写出的稿子可直接粘进起草页。
- **数据不出本机**：默认连接本机 LM Studio / Ollama（OpenAI 兼容接口），正文不会主动发送到互联网，涉密单位可用；要过网关时可在设置里选直连 / 跟随系统 / 自定义代理（HTTP / HTTPS / SOCKS），本机地址始终直连。
...
<table>
  ...
  <tr>
    ...
    <td valign="top"><strong>改了什么，一眼说清</strong><br/>左栏是可编辑的统一 diff——删除的旧行留在原位、新行铺绿底，每个变更块一键还原；右栏同步出花脸稿预览，可回退基准版，或导出符合送审习惯的花脸稿。</td>
  </tr>
  <tr>
...
```text
src/                应用代码（约 11.8 万行 Rust，1250+ 自动化测试）
assets/             图标与界面资源
examples/           示例公文
