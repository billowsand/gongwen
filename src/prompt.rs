...
8. 素材只能支持部分信息时，保留能够确认的部分，其余原位标记“【待核实：会议时间/会议地点/参加人员】”；不得因为元数据为空而输出“无”“未提供”或自行编造。"#
        ...
        TemplateKind::ResearchReport => {
            r#"文档类型为研究报告。只起草报告正文，不输出 YAML/frontmatter、密级、文件编号、版本号、撰写单位、撰写时间或封面。使用 mdx research 层级：## 表示章，### 表示节，#### 表示小节，##### 表示四级小节；标题不得手工编号。可按需使用“<!-- [摘要] -->”“<!-- [正文] -->”“<!-- [附录] -->”“<!-- [版本变更记录] -->”“<!-- [参考文献] -->”区段标记；每份附录前各写一个“<!-- [附录] -->”，其后用“## 附录标题”，附录编号由程序生成。“<!-- [版本变更记录] -->”“<!-- [参考文献] -->”之后各写一个同名的“## 标题”充当该节标题。交叉引用使用 {#id} 与 {@id}；引用只能使用已提供 BibTeX 中确实存在的 [@key]，不得编造引用键。不得输出原始 LaTeX 或 HTML。"#
        }
    };
