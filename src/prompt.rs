...
8. 素材只能支持部分信息时，保留能够确认的部分，其余原位标记“【待核实：会议时间/会议地点/参加人员】”；不得因为元数据为空而输出“无”“未提供”或自行编造。"#
        ...
        TemplateKind::ResearchReport => {
            r#"文档类型为研究报告。只起草报告正文，不输出 YAML/frontmatter、密级、文件编号、版本号、撰写单位、撰写时间或封面。报告题名在文首以“# 报告题名”输出，全文至多一个，文字须与资料中给出的文件名称一致（题名印在封面上，正文版面不再重复）；章从“##”开始。使用 mdx research 层级：## 表示章，### 表示节，#### 表示小节，##### 表示四级小节；标题不得手工编号。可按需使用“<!-- [摘要] -->”“<!-- [正文] -->”“<!-- [附录] -->”“<!-- [版本变更记录] -->”“<!-- [参考文献] -->”区段标记；每份附录前各写一个“<!-- [附录] -->”，其后用“## 附录标题”，附录编号由程序生成。“<!-- [版本变更记录] -->”“<!-- [参考文献] -->”之后各写一个同名的“## 标题”充当该节标题。交叉引用使用 {#id} 与 {@id}；引用只能使用已提供 BibTeX 中确实存在的 [@key]，不得编造引用键。不得输出原始 LaTeX 或 HTML。"#
        }
    };
...
正文从“<!-- [摘要] -->”或“<!-- [正文] -->”区段开始，章标题使用“## 标题”，节标题使用“### 标题”，依次类推，标题文本不要手工编号。附录与公文附件写法一致：每份附录前各写一个“<!-- [附录] -->”标识。参考文献只写正文引用，文献表由 BibTeX 自动生成。"#
            .to_string(),
    };
...
正文一级标题使用“## 标题”、二级标题使用“### 标题”，标题文本不要手写编号；正文区段用“<!-- [正文] -->”标记；有附件内容时，每份附件前重复独占一行的“<!-- [附件] -->”，下一行用“# 附件正式标题”；附件内部标题与正文使用相同层级，附件编号由程序生成，不要手写。不得输出批示框、发文字号、承办信息或落款。"#
        ...
        TemplateKind::ResearchReport => {
            r#"只保留研究报告正文，不输出 YAML/frontmatter 和封面要素；文首可保留至多一个“# 报告题名”（与文件名称同文，只供封面取用），##、###、####、##### 依次表示章、节、小节和四级小节，标题不要手工编号。保留 mdx research 区段标记、{#id}/{@id} 交叉引用和 [@key] BibTeX 引用语法；不得编造引用键。"#
        }
    }
}
...
2. 报告题名以“# 报告题名”写在文首，全文至多一个，文字与文件名称一致；章从“## 标题”开始，节、小节和四级小节依次使用“###”“####”“#####”，标题文本不得手工编号。
3. 可保留 mdx research 区段标记：“<!-- [摘要] -->”“<!-- [正文] -->”“<!-- [附录] -->”“<!-- [版本变更记录] -->”“<!-- [参考文献] -->”；原文有“<!-- [目录] -->”的原位保留，原文没有的不要添加；每份附录前各写一个“<!-- [附录] -->”标识，“<!-- [版本变更记录] -->”“<!-- [参考文献] -->”之后各写一个同名的“## 标题”。
4. 图片使用标准 Markdown 图片语法；表格使用标准 Markdown 表格，各行列数一致；图题写在图片的替代文本里，表题写成表格上一行的“表：题名”。
