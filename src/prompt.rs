...
pub fn build_draft_prompt(
    ...
    let rules = match input.kind {
        TemplateKind::OfficialLetter => {
            r#"文种为公函。标题通常为“关于……的函”。正文交代依据、事项和明确请求；不得凭空补齐缺失事实。根据语义选择“特此函告”“特此函复”或“特此函商，请予支持为荷”等规范结语。正文一级标题使用“## 标题”（导出时自动转为“一、二、三……”黑体），正文二级标题使用“### 标题”（自动转为“（一）（二）（三）……”楷体）；标题文本中不要手写编号。用户素材明确要求附带具体附件内容时，使用独占一行的“<!-- [附件] -->”切换附件区段。每个附件依次使用“# 附件1”“# 附件2”作为附件标识，下一行使用“## 附件正式标题”；不得把附件名称写在“# 附件1”同一行，也不得把仅在正文中提到的附件、附件说明或报送表名称臆造成附件全文。附件表格使用标准 Markdown 表格。"#
        }
        TemplateKind::PhoneNotice => {
            r#"文种为电话通知。版式与公函一致，但不设置机关代字、发文字号及底部版记。标题通常为“关于……的通知”。正文应明确通知事项、执行要求和时间节点；不得凭空补齐缺失事实，结尾根据语义使用“特此通知”等规范表述。正文一级标题使用“## 标题”（导出时自动转为“一、二、三……”黑体），正文二级标题使用“### 标题”（自动转为“（一）（二）（三）……”楷体）；标题文本中不要手写编号。用户素材明确要求附带具体附件内容时，使用独占一行的“<!-- [附件] -->”切换附件区段。每个附件依次使用“# 附件1”“# 附件2”作为附件标识，下一行使用“## 附件正式标题”；附件表格使用标准 Markdown 表格。"#
        }
        TemplateKind::PlainDocument => {
            r#"文种为普通公文。成稿不设置红头、发文单位、主送单位、落款、成文日期和版记，只排标题、正文及可选附件。正文应准确、简明、规范，不得凭空补齐缺失事实。正文一级标题使用“## 标题”（导出时自动转为“一、二、三……”黑体），正文二级标题使用“### 标题”（自动转为“（一）（二）（三）……”楷体）；标题文本中不要手写编号。用户素材明确要求附带具体附件内容时，使用独占一行的“<!-- [附件] -->”切换附件区段。每个附件依次使用“# 附件1”“# 附件2”作为附件标识，下一行使用“## 附件正式标题”；附件表格使用标准 Markdown 表格。"#
        }
...
正文区段用“<!-- [正文] -->”标记；有附件内容时，在正文结束后用独占一行的“<!-- [附件] -->”切换附件区段，每个附件用“# 附件1”“# 附件2”标识。"#
        }
        TemplateKind::MeetingAgenda => {
...
/// 只认标题文字恰好是“正文”或“附件”的行，“# 附件1”是真附件标识，必须留下。
