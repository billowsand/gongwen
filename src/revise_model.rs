...
pub const TASKS: [ReviseTask; 3] = [
    ...
    ReviseTask {
        ...
        // 单句里根本看不出来，问了只会让模型瞎猜。
        criteria: "称谓不规范：使用「你们」「咱们」「大家」等口语称谓；对不相隶属机关未用「贵」等敬称；提及个人时未加「同志」或职务",
        // 实测抓到的过度适用：模型学会「敬称用贵」之后到处套，连自称「我办」和
        // 下行文里的「你单位」都要改成「贵」。GC-109 三跑三中，是稳定误报。
        exclusions: "「我局」「我办」「我单位」是规范的自称，绝不能改成「贵」；上级机关对下级行文（批复、通知等）称对方「你局」「你单位」也是规范的，同样不能改成「贵」。「贵」只用于不相隶属机关之间的平行文",
    },
    ReviseTask {
        id: "MDL-PUNCT",
        label: "标点规范",
...
/// 去赌一条没把握的。
///
/// 实测（77 条回归集，每句跑 3 遍取区间）：
/// - 语病与表达 召回 100%~100%、误报 0%~0% → 默认开
/// - 标点规范　 召回 80%~90%、 误报 0%~0% → 默认开
/// - 称谓规范　 召回 80%~90%、 误报 3%~6% → 默认关
///
/// 称谓这一条上一版曾默认开，是错的：当时的「误报 0/28」来自单跑一遍，
/// 而跑三遍后暴露出一条**三跑三中**的稳定误报（把下行文里规范的「你单位」
/// 改成「贵」）。已写进 `exclusions` 补掉，但**补丁有没有生效要重新量过才算数**，
/// 在那之前退回默认关闭。
///
