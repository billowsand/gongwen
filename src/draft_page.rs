...
use crate::{
    app::{
        ...
        FORM_PANEL_MIN_WIDTH, LABEL_WIDTH, SelectOption, VersionScope,
        ...
        reveal_in_os, row_label, row_label_with_info, single_select,
...
impl FormSection {
    ...
    fn hint(self, kind: TemplateKind) -> &'static str {
        match (self, kind) {
...
pub(crate) struct FormCheck {
    progress: [SectionProgress; 3],
    errors: std::collections::BTreeMap<&'static str, String>,
}

impl FormCheck {
    /// 记一个必填项。`filled` 为假时把说明挂到字段 id 上，渲染时就地显示。
    fn require(&mut self, section: FormSection, id: &'static str, filled: bool, message: &str) {
...
fn check_form(draft: &DraftInput) -> FormCheck {
...
/// 不该等到点了生成、在右侧抽屉里才发现。
fn required_row_label(ui: &mut egui::Ui, label: &str, tip: Option<&str>) {
...
/// 它自己占满一整行（标签列留空）。
fn field_error(ui: &mut egui::Ui, check: &FormCheck, id: &str) {
...
fn layout_thumbnail(ui: &mut egui::Ui, kind: TemplateKind, check: &FormCheck) -> Option<FormSection> {
