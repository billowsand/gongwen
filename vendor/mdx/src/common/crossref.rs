...
//!
//! 已知限制：不模拟区段标记模式——摘要/版本变更记录/参考文献段内标题、以及
//! 正文区的 H1（报告题名，只上封面不排版面）上的锚点会被 emitter 丢弃，但本
//! 检查仍视为已定义（此类引用最终由 LaTeX 报 undefined reference）。区段是由
//! `<!-- [...] -->` 标记切出来的，同一个 H1 在附录段里又是合法的附录章标题，
//! 这里不跟着走一遍标记就分不开，故留给调用方（gongwen 的校验器按区段判定）。
